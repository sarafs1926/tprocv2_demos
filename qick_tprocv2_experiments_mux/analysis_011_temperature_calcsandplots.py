...
temperature_folder = os.path.join(outerFolder, "Temperatures")
    ...
    for q_key in populated_keys:
        ...
        for dataset in range(len(load_data['SS'][q_key].get('Dates', [])[0])):
            ...
            if len(Q_g)>0:
                ...
                ground_state_population, excited_state_population_overlap, gmm, means, covariances, weights, crossing_point, ground_gaussian, excited_gaussian, ground_data, excited_data, iq_data = fit_double_gaussian_with_full_coverage(
                    ig_new)
                temperature_k = calculate_qubit_temperature(qubit_frequency, ground_state_population,
