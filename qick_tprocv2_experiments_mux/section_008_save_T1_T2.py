    ...
    def save_to_h5(self, t_type):
        ...
        with h5py.File(h5_filename, 'w') as f:
            ...
            for QubitIndex, data in self.data.items():
                ...
                group.create_dataset("I", data=np.array(data['I'], dtype='S'))
                group.create_dataset("Q", data=np.array(data['Q'], dtype='S'))
                group.create_dataset("Delay Times", data=np.array(data['Delay Times']))
                group.create_dataset("Fit", data=np.array(data['Fit']))
                group.create_dataset("Round Num", data=np.array(data['Round Num']))
                group.create_dataset("Batch Num", data=np.array(data['Batch Num']))
